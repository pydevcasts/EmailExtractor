...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.rm_db_label.grid(row=3, column=0, pady=10, padx=10)
        self.button_2 = customtkinter.CTkButton(master=self.frame_left, image=self.rm_db,
                                                text="",
                                                fg_color=("gray75", "gray30"),  # <- custom tuple-color
                                                command=lambda: asyncio.run(clearData())
                                                )
        self.button_2.grid(row=3, column=0, pady=10, padx=20)
        # self.tab_view = MyTabView(master=self.frame_left)
